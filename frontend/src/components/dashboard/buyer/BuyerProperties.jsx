...
// Fallback mock data when no connection
...
const BuyerProperties = () => {
  ...
  useEffect(() => {
    ...
    if (listingType !== 'all') filtered = filtered.filter(prop => prop.listing_type === listingType);
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter(prop => prop.title.toLowerCase().includes(term) || prop.city.toLowerCase().includes(term));
    }
    if (priceRange !== 'all') {
      if (listingType === 'rent') {
        ...
        else if (priceRange === 'above-15m') filtered = filtered.filter(prop => prop.price > 15000000);
      }
    }
    if (bedrooms !== 'any') filtered = filtered.filter(prop => prop.bedrooms >= parseInt(bedrooms));
    if (bathrooms !== 'any') filtered = filtered.filter(prop => prop.bathrooms >= parseInt(bathrooms));
    if (selectedLocation !== 'all') filtered = filtered.filter(prop => prop.city === selectedLocation);
    if (sortBy === 'price_low') filtered.sort((a, b) => a.price - b.price);
    else if (sortBy === 'price_high') filtered.sort((a, b) => b.price - a.price);
    else if (sortBy === 'latest') filtered.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    ...
    <div>
      <div className="mb-6">
        <div className="flex gap-2">
          ...
          {hasActiveFilters && (
            <button onClick={resetFilters} className="px-4 py-2.5 rounded-xl text-sm bg-red-50 text-red-600 hover:bg-red-100">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              ...
              <select value={listingType} onChange={(e) => setListingType(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm">
            ...
            <div>
              ...
              <select value={priceRange} onChange={(e) => setPriceRange(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm">
                <option value="all">Any Price</option>
                {listingType === 'rent' ? (
                  <>
                    <option value="under-20k">Under ETB 20,000</option>
                    <option value="20k-50k">ETB 20,000 - 50,000</option>
                    <option value="above-50k">Above ETB 50,000</option>
                  </>
                ) : (
                  <>
                    <option value="under-5m">Under ETB 5M</option>
                    <option value="5m-15m">ETB 5M - 15M</option>
                    <option value="above-15m">Above ETB 15M</option>
                  </>
                )}
              </select>
            </div>
            <div>
              ...
              <select value={bedrooms} onChange={(e) => setBedrooms(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm">
            ...
            <div>
              ...
              <select value={bathrooms} onChange={(e) => setBathrooms(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm">
                <option value="any">Any</option>
            ...
            <div>
              ...
              <select value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm">
      ...
      <div className="flex justify-between items-center mb-4">
        <p className="text-sm text-gray-500">{filteredProperties.length} properties found</p>
        <div className="flex gap-2">
          <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
            <Grid3X3 className="w-4 h-4" />
          </button>
          <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
      ...
      {filteredProperties.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
          ...
          {hasActiveFilters && <button onClick={resetFilters} className="mt-3 text-blue-600 text-sm hover:underline">Clear all filters</button>}
        </div>
      ) : viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredProperties.map((property) => (
            <div key={property.id} onClick={() => navigate(`/properties/${property.id}`)} className="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-lg transition-all cursor-pointer group">
              <div className="relative h-44 bg-gray-200">
                ...
                {property.featured && <div className="absolute top-3 left-3 px-2 py-1 rounded-lg text-xs font-semibold bg-yellow-500 text-white flex items-center gap-1"><Star className="w-3 h-3" /> Featured</div>}
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-gray-900 group-hover:text-blue-600 transition line-clamp-1">{property.title}</h3>
                <div className="flex items-center gap-1 text-gray-500 text-sm mt-1"><MapPin className="w-3 h-3" /> {property.city}</div>
                ...
                <button onClick={(e) => { e.stopPropagation(); handleSaveProperty(property); }} className={`mt-3 w-full py-2 rounded-lg text-sm font-medium transition ${
                  savedProperties.some(p => p.id === property.id) ? 'bg-gray-100 text-gray-600' : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}>
        ...
        <div className="space-y-3">
          {filteredProperties.map((property) => (
            <div key={property.id} onClick={() => navigate(`/properties/${property.id}`)} className="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition cursor-pointer flex">
              ...
              <div className="flex-1 p-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <span className={`inline-block mt-1 px-2 py-1 rounded-lg text-xs font-semibold ${property.listing_type === 'sale' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
