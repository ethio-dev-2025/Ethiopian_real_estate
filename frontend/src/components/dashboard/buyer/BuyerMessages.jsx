...
import { useAuth } from '../../../context/AuthContext'  // ← CORRECT PATH: 3 levels up to src/context
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredConversations.length === 0 ? (
            ...
            filteredConversations.map(conv => (
              <div key={conv.id} onClick={() => { setSelectedConversation(conv); fetchMessages(conv.user_id); navigate(`/dashboard/buyer/messages/${conv.id}`) }} className={`p-3 flex gap-3 cursor-pointer hover:bg-gray-50 transition ${selectedConversation?.user_id === conv.user_id ? 'bg-blue-50' : ''}`}>
