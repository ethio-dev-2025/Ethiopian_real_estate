...
const MessagesPage = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'delivered': return <CheckCheck className="w-3 h-3 text-gray-300" />
      case 'sent': return <Check className="w-3 h-3 text-gray-300" />
  ...
  // DEFINE fetchMessages FIRST before using it
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('📋 Fetched conversations:', sorted.length)
      }
    } catch (err) {
      console.error(err)
    }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const endpoints = [
        `${API_URL}/api/messages/upload-attachment`,
        `${API_URL}/api/listings/upload-image`
      ]
  ...
  // Auto open chat from property contact (location state)
  useEffect(() => {
  ...
  }, [socket, selectedConversation, addMessageHandler, user, fetchConversations])
  ...
  const sendMessage = async (file = null, fileType = null) => {
    ...
    if (file) {
      ...
      if (!inputMessage && fileType === 'image') {
    ...
    const optimisticMessage = {
      ...
      attachment_type: file?.type?.startsWith('image/') ? 'image' : (fileType || 'file'),
      status: 'sending',
            ...
            filteredConversations.map(conv => (
              <div
                ...
                onClick={() => {
                  setSelectedConversation(conv)
                  fetchMessages(conv.user_id)
                  navigate(`/dashboard/buyer/messages/${conv.id}`)
                }}
                className={`p-3 flex gap-3 cursor-pointer hover:bg-gray-50 transition ${selectedConversation?.user_id === conv.user_id ? 'bg-blue-50' : ''}`}
              >
          ...
          <>
            ...
            <div className="bg-white border-t p-4">
              ...
              <div className="flex gap-2 items-center">
                <input type="file" ref={fileInputRef} onChange={(e) => handleFileSelect(e, 'file')} className="hidden" accept=".pdf,.doc,.docx,.xlsx,.pptx,.txt" />
                ...
                <textarea
                  ...
                  disabled={sending}
                />
                <button onClick={() => sendMessage()} disabled={(!inputMessage.trim() && !attachment) || sending} className="bg-blue-600 text-white p-3 rounded-lg hover:bg-blue-700 disabled:opacity-50">
