...
const VerificationQueue = () => {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'pending':
        return <span className="inline-flex items-center gap-1 px-2 py-1 bg-secondary-100 text-secondary-700 rounded-full text-xs"><Clock className="w-3 h-3" />Pending</span>
      case 'approved':
        return <span className="inline-flex items-center gap-1 px-2 py-1 bg-success/10 text-success rounded-full text-xs"><CheckCircle className="w-3 h-3" />Approved</span>
      case 'rejected':
        return <span className="inline-flex items-center gap-1 px-2 py-1 bg-error/10 text-error rounded-full text-xs"><XCircle className="w-3 h-3" />Rejected</span>
      default:
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b p-4 flex justify-between items-center">
          <h2 className="text-xl font-bold flex items-center gap-2"><Users className="w-5 h-5 text-primary-700" />Activation Request Details</h2>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg"><XCircle className="w-5 h-5" /></button>
        </div>
        
        <div className="p-6 space-y-6">
          <div className="border-b pb-4">
            <h3 className="font-semibold text-lg mb-3 flex items-center gap-2"><UserCheck className="w-4 h-4 text-primary-700" />Personal Information</h3>
          ...
          <div className="border-b pb-4">
            <h3 className="font-semibold text-lg mb-3 flex items-center gap-2"><Building2 className="w-4 h-4 text-primary-700" />Property Information</h3>
          ...
          {(request.business_name || request.reason_for_activation) && (
            <div className="border-b pb-4">
              <h3 className="font-semibold text-lg mb-3 flex items-center gap-2"><Briefcase className="w-4 h-4 text-primary-700" />Additional Information</h3>
              <div className="space-y-2">
          ...
          {request.status === 'pending' && (
            <div className="flex gap-3 pt-4 border-t">
              <button onClick={() => handleApprove(request.id)} className="flex-1 px-4 py-2 bg-success text-white rounded-lg font-semibold hover:bg-green-700 flex items-center justify-center gap-2">
              ...
              <button onClick={() => {
                ...
                onClose()
              }} className="flex-1 px-4 py-2 bg-error text-white rounded-lg font-semibold hover:bg-red-700 flex items-center justify-center gap-2">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => handleReject(selectedRequest?.id)} className="flex-1 px-4 py-2 bg-error text-white rounded-lg">Confirm Reject</button>
        </div>
      </div>
    </div>
  )
      ...
      <main className={`transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-20'}`}>
        ...
        <div className="p-6">
          <div className="flex gap-2 mb-6 border-b">
            <button onClick={() => setFilter('pending')} className={`px-4 py-2 font-medium transition ${filter === 'pending' ? 'border-b-2 border-primary-700 text-primary-700' : 'text-gray-500'}`}>
              Pending ({pendingCount})
            </button>
            <button onClick={() => setFilter('all')} className={`px-4 py-2 font-medium transition ${filter === 'all' ? 'border-b-2 border-primary-700 text-primary-700' : 'text-gray-500'}`}>
          ...
          {error ? (
            <div className="bg-error/10 border border-error/20 rounded-2xl p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-error mb-2">Error Loading Requests</h3>
              <p className="text-error mb-4">{error}</p>
              <button onClick={fetchRequests} className="px-4 py-2 bg-error text-white rounded-lg hover:bg-red-700">
                Try Again
              </button>
            </div>
          ) : requests.length === 0 ? (
                ...
                <div key={req.id} className="bg-white rounded-2xl shadow-sm border p-6 hover:shadow-md transition">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2 ml-4">
                      ...
                      {req.status === 'pending' && (
                        <>
                          <button onClick={() => handleApprove(req.id)} className="px-4 py-2 bg-success text-white rounded-lg text-sm font-medium hover:bg-green-700 flex items-center gap-1">
                            <Check className="w-4 h-4" /> Approve
                          </button>
                          <button onClick={() => {
                            setSelectedRequest(req)
                            setShowRejectModal(true)
                          }} className="px-4 py-2 bg-error text-white rounded-lg text-sm font-medium hover:bg-red-700 flex items-center gap-1">
